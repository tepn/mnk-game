...
board_new(const int m, const int n)
{
	...
	board->player1->rotate0 = malloc(m * sizeof(uint64_t));
	...
	board->player1->rotate90 = malloc(n * sizeof(uint64_t));
	...
	board->player2->rotate0 = malloc(m * sizeof(uint64_t));
	...
	board->player2->rotate90 = malloc(n * sizeof(uint64_t));
...
board_win(const board_t* board, const int k)
{
	...
	if ((k < 1) || (k > BOARD_MAX) || (k > board->m) || (k > board->n))
	{
		return -1;
	}
